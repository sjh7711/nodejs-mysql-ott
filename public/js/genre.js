...
const pagenationBt = async direction => {
  ...
  const res = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${api_key}&language=ko&sort_by=popularity.desc&include_adult=false&include_video=false&page=${pageCount}&with_genres=${selectedGenre.id}`);
...
window.onload = async () => {
  ...
  while(i <= 5) {
    const res = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${api_key}&language=ko&sort_by=popularity.desc&include_adult=false&include_video=false&page=${i}with_genres=${selectedGenre.id}`);
...
$pagenation.onclick = async e => {
  ...
  // 영화 이미지, 타이틀 렌더
  const res = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${api_key}&language=ko&sort_by=popularity.desc&include_adult=false&include_video=false&page=${e.target.textContent}&with_genres=${selectedGenre.id}`);
  const { results: movies } = await res.json();
  $main__name.textContent = selectedGenre.name;
  movies.forEach(movie => render(movie));
  $result__movies.appendChild($fragment);
